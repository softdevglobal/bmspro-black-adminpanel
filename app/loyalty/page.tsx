...
export default function LoyaltyPage() {
  ...
  const showToast = (msg: string) => {
    ...
    toast.className = "fixed bottom-4 right-4 bg-neutral-900 text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-3 z-50 animate-slide-in";
    toast.innerHTML = `<i class="fas fa-gem text-amber-500"></i><span>${msg}</span>`;
          ...
          {loading ? (
            <div className="flex items-center justify-center h-64">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-neutral-500 font-medium">Loading loyalty settings...</p>
              </div>
            </div>
          ) : (
            <>
                  ...
                  <div className="relative flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl bg-amber-500/20 flex items-center justify-center border border-amber-500/30">
                        <i className="fas fa-gem text-amber-400" />
                      </div>
                      <div>
                        <h1 className="text-2xl font-bold">ACSU Loyalty</h1>
                        <p className="text-sm text-neutral-400 mt-1">Manage loyalty points and rewards</p>
                      </div>
                    </div>
              ...
              <section>
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-neutral-800">ACSU Loyalty</h2>
                    <p className="text-sm text-neutral-500">Configure your reward logic and track distributed points.</p>
                  </div>
                  <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-neutral-200">
                    <span className="text-xs font-bold text-neutral-500 uppercase">ACSU Integration:</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                  <div className="bg-neutral-900 text-white rounded-2xl p-6 border-none flex flex-col justify-between overflow-hidden relative">
                    <div className="relative z-10">
                      <p className="text-neutral-400 text-xs font-bold uppercase tracking-wider">ACSU Points Balance</p>
                      <h3 className="text-4xl font-black mt-1">{acsuData.balance.toLocaleString()}</h3>
                    </div>
                    <button
                      onClick={() => setShowTopupModal(true)}
                      className="relative z-10 w-full mt-6 bg-neutral-900 hover:bg-neutral-800 text-white py-2.5 rounded-lg text-sm font-bold transition"
                    >
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-neutral-200 shadow-sm lg:col-span-2">
                    <h3 className="font-bold text-neutral-800 mb-4">Dollar to ACSU Point Value</h3>
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                      <div className="md:col-span-4">
                        <label className="block text-xs font-bold text-neutral-500 mb-1 uppercase">Currency</label>
                        <div className="relative">
                          <select
                            id="acsu-currency"
                            defaultValue={acsuData.currency}
                            disabled={!acsuData.isEnabled || saving}
                            className="w-full border border-neutral-200 rounded-lg p-2 pr-10 text-sm outline-none focus:ring-2 focus:ring-neutral-900 disabled:opacity-50 disabled:cursor-not-allowed appearance-none"
                          >
                            <option value="AUD">AUD - Australian Dollar</option>
                            <option value="USD">USD - US Dollar</option>
                            <option value="LKR">LKR - Sri Lankan Rupee</option>
                          </select>
                          <i className="fas fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none text-xs" />
                        </div>
                      </div>
                      <div className="md:col-span-5">
                        <label className="block text-xs font-bold text-neutral-500 mb-1 uppercase">Points per $1.00 Value</label>
                        <input
                          type="number"
                          id="acsu-point-value"
                          defaultValue={acsuData.conversionRate}
                          disabled={!acsuData.isEnabled || saving}
                          className="w-full border border-neutral-200 rounded-lg p-2 text-sm outline-none focus:ring-2 focus:ring-neutral-900 disabled:opacity-50 disabled:cursor-not-allowed"
                          placeholder="10"
                        />
                      </div>
                      <div className="md:col-span-3 flex items-end">
                        <button
                          ...
                          disabled={!acsuData.isEnabled || saving}
                          className="w-full bg-neutral-800 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-black transition disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          Update Formula
                        </button>
                      </div>
                ...
                <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden">
                  <div className="p-4 border-b border-neutral-100 bg-neutral-50">
                    <h3 className="font-bold text-neutral-800">Transaction Records</h3>
                  </div>
                  {transactions.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-sm whitespace-nowrap">
                        <thead className="bg-white text-neutral-400 font-bold border-b uppercase text-[10px] tracking-widest">
                          <tr>
                            ...
                            <th className="p-4 text-neutral-700">ACSU Points</th>
                            <th className="p-4 text-right">Date</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-neutral-50">
                          {transactions.map((t) => (
                            <tr key={t.id} className="hover:bg-neutral-50 transition border-b border-neutral-50 last:border-none">
                              <td className="p-4 text-neutral-400 font-mono text-[11px]">{t.email}</td>
                              <td className="p-4 font-bold text-neutral-800">{t.name}</td>
                              <td className="p-4 text-neutral-600">{t.staff}</td>
                              <td className="p-4">
                                <span className="bg-neutral-100 px-2 py-0.5 rounded text-[10px] font-bold">{t.branch}</span>
                              </td>
                              <td className="p-4 text-neutral-600">{t.service}</td>
                              <td className="p-4 font-medium">
                                {acsuData.currency} ${t.value}
                              </td>
                              <td className="p-4 font-black text-neutral-700">+{t.points.toLocaleString()}</td>
                              <td className="p-4 text-right text-neutral-400 text-xs">
                      ...
                      <form onSubmit={handleACSUTopup} className="p-6 space-y-4">
                        ...
                        <div className="bg-neutral-50 rounded-lg p-4 border border-neutral-200">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-bold text-neutral-500 uppercase">Current Balance</span>
                            <span className="text-2xl font-black text-neutral-900">{acsuData.balance.toLocaleString()}</span>
                          </div>
                          {topupAmount && !isNaN(parseInt(topupAmount)) && parseInt(topupAmount) > 0 && (
                            <div className="flex items-center justify-between pt-2 border-t border-neutral-200">
                              <span className="text-xs font-bold text-neutral-500 uppercase">New Balance</span>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            required
                            disabled={saving}
                            className="w-full border border-neutral-300 rounded-lg p-3 text-2xl font-black text-center focus:ring-2 focus:ring-neutral-900 outline-none disabled:opacity-50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            placeholder="1000"
                          />
                        </div>
                        ...
                        <button
                          ...
                          className="w-full bg-neutral-900 text-white font-bold py-3 rounded-lg shadow-lg hover:bg-black transition disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          {saving ? (
                            <>
