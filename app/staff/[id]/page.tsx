    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      <div className="lg:col-span-2 bg-white rounded-2xl border border-neutral-200 shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="text-sm font-bold text-neutral-700">Staff Calendar</div>
            <p className="text-xs text-neutral-500 mt-1">Click a date to view appointments</p>
          </div>
        ...
        <div className="flex items-center gap-4 mb-3 text-xs text-neutral-500">
          ...
          <div className="flex items-center gap-1">
            <div className="w-4 h-4 rounded bg-neutral-100 text-neutral-600 text-[10px] flex items-center justify-center font-bold">3</div>
            <span>Bookings</span>
          </div>
        </div>
                  ...
                  {c.date && (
                    <>
                      <div className="flex items-start justify-between">
                        ...
                        <span
                          className={`w-2 h-2 rounded-full ${c.isWorking ? "bg-emerald-400" : "bg-neutral-300"}`}
      ...
      <div className="lg:col-span-3 space-y-4">
        ...
        <div className={`rounded-2xl p-5 ${
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className={`text-lg font-bold ${!isWorkingDay ? "text-neutral-700" : ""}`}>
                {selectedDate.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" })}
                {isToday && <span className="ml-2 text-xs bg-white/20 px-2 py-0.5 rounded-full">Today</span>}
              </div>
              <div className={`text-sm mt-1 ${!isWorkingDay ? "text-neutral-500" : "text-white/80"}`}>
                {isWorkingDay ? (
            ...
            <div className={`text-right ${!isWorkingDay ? "text-neutral-600" : ""}`}>
        ...
        {isWorkingDay && (
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-white rounded-xl border border-neutral-200 p-3 text-center">
              <div className="text-xl font-bold text-neutral-600">{completedCount}</div>
        ...
        <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-4">
          <div className="text-sm font-bold text-neutral-700 flex items-center gap-2 mb-3">
            <i className="fas fa-list-check text-neutral-500" />
            Day's Appointments
          </div>
          ...
          {!isWorkingDay ? (
            ...
            <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
              {sortedBookings.map((b, idx) => {
                ...
                const statusColor = statusColors[b.status] || "bg-neutral-50 border-neutral-200 text-neutral-600";
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-center shrink-0 w-14">
                          ...
                          <div className="text-[10px] text-neutral-400">{b.duration ? `${b.duration}min` : ""}</div>
                        </div>
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-neutral-600 mt-2 font-semibold">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <main className="flex-1 overflow-auto p-4 sm:p-6 lg:p-8">
          ...
          <div className="max-w-7xl mx-auto">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-sm font-medium transition mr-1 mb-1 ${
                    activeTab === t.key ? "bg-neutral-900 text-white shadow" : "text-neutral-600 hover:bg-neutral-100"
                  }`}
                >
            ...
            {loading ? (
              <div className="text-neutral-500">Loading staff…</div>
            ) : !staff ? (
              ...
              <>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {staff.email && (
                              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-neutral-100 truncate">
                                <i className="fas fa-envelope text-neutral-600" /> {staff.email}
                              </div>
                            )}
                            {staff.branchName && (
                              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-neutral-100">
                                <i className="fas fa-store text-neutral-600" /> {staff.branchName}
                              </div>
                            )}
                          </div>
                        </div>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {isWorking ? (
                                    ...
                                    <div className="text-[10px] text-neutral-400">Off</div>
                                  )}
                                </div>
                              );
                            })}
                ...
                {activeTab === "training" && (
                  <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-6">
                    <div className="text-sm font-bold text-neutral-700 mb-4">Training Matrix</div>
                    <div className="flex gap-3">
                      <span className={`px-3 py-1.5 rounded-full text-xs ${staff.training?.ohs ? "bg-emerald-100 text-emerald-700" : "bg-neutral-100 text-neutral-600"}`}>
                        OHS
                      </span>
                      <span className={`px-3 py-1.5 rounded-full text-xs ${staff.training?.prod ? "bg-emerald-100 text-emerald-700" : "bg-neutral-100 text-neutral-600"}`}>
                        Product
                      </span>
                      <span className={`px-3 py-1.5 rounded-full text-xs ${staff.training?.tool ? "bg-emerald-100 text-emerald-700" : "bg-neutral-100 text-neutral-600"}`}>
                        Tools
                      </span>
                    </div>
                  </div>
                ...
                {activeTab === "schedule" && (
                  <>
                    {!staff.weeklySchedule || Object.keys(staff.weeklySchedule).length === 0 ? (
                      <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm p-6 text-sm text-neutral-500">
                        ...
                        <table className="w-full text-left border-collapse">
                          ...
                          <tbody className="divide-y divide-neutral-100 text-sm">
                            {staffBookings.slice(0, 20).map((b, idx) => {
                              ...
                              const statusColor = statusColors[b.status] || "bg-neutral-50 text-neutral-600 border-neutral-200";
                              
                              return (
                                <tr key={b.id || idx} className="hover:bg-neutral-50 transition-colors">
                                  <td className="p-3">
                                  ...
                                  <td className="p-3">
                                    <div className="flex items-center gap-2">
                                      <div className="w-7 h-7 rounded-full bg-neutral-100 text-neutral-600 flex items-center justify-center text-xs font-semibold">
                                        {(b.client || "U").toString().slice(0, 1).toUpperCase()}
                                      </div>
                                      <span className="text-neutral-700">{b.client || "Unknown"}</span>
                                    </div>
                                  </td>
                                  <td className="p-3 text-neutral-600">{b.serviceName || "Service"}</td>
                                  <td className="p-3 font-semibold text-neutral-700">${Number(b.price || 0).toFixed(0)}</td>
                                  <td className="p-3">
                      ...
                      return (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {customers.slice(0, 12).map((c, idx) => (
                            <div key={idx} className="border border-neutral-200 rounded-xl p-4 hover:shadow-md transition bg-white">
                              <div className="flex items-start gap-3">
                                <div className="w-10 h-10 rounded-full bg-neutral-100 text-neutral-600 flex items-center justify-center font-semibold">
                                ...
                                <div className="mt-3 pt-3 border-t border-neutral-100 text-xs text-neutral-500 space-y-1">
                                  {c.email && (
                                    <div className="flex items-center gap-2 truncate">
                                      <i className="fas fa-envelope text-neutral-400" />
                                      {c.email}
                                    </div>
                                  )}
                                  {c.phone && (
                                    <div className="flex items-center gap-2">
                                      <i className="fas fa-phone text-neutral-400" />
                                      {c.phone}
                                    </div>
                                  )}
                                </div>
