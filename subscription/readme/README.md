...
| **Super admin creates/edits plans** | UI: `/packages` → API: `/api/packages` |
| **Super admin onboards tenants with a plan** | UI: `/tenants` (writes directly to `users`) |
| **Workshop owner pays / upgrades** | UI: `/subscription` → Stripe Checkout → `/api/stripe/webhook` |

There is **no** Firestore collection named `subscriptions`. Plans are catalog documents; live subscription status is stored on each workshop owner’s user document and synced from Stripe.
...
Super Admin logs in
    ...
    → POST /api/packages with Authorization: Bearer {idToken}
    ...
    → returns { id, plan }
    → UI refreshes GET /api/packages
```

**Entry point UI:** `app/packages/page.tsx`  
**API:** `app/api/packages/route.ts` (POST handler)  
**Helpers:** `lib/subscriptionPlans.ts` (`normalizeBillingCycle`, `validityDaysForCycle`, labels)
...
- **`/tenants`:** create tenant or edit tenant → sets `planId`, `plan_key`, `branchLimit`, `staffLimit`, trial fields
...
Super admin completes 3-step modal
    ...
    → setDoc(users/{ownerUid}, { planId, plan, branchLimit, staffLimit, trial fields, ... })
...
- `app/packages/page.tsx` — Packages UI (CRUD + tenant plan override)
- `app/api/packages/route.ts` — CRUD for `subscription_plans`
